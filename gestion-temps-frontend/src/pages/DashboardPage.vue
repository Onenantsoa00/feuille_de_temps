...
const missionDeadlines = computed(() => {
  ...
  return (stats.value.missionDeadlines || []).map((row) => {
    const end = row.end_date
      ? new Date(String(row.end_date).includes('T') ? row.end_date : `${row.end_date}T12:00:00`)
      : null
    const dayMs = 24 * 60 * 60 * 1000
    const delta =
      end && !Number.isNaN(end.getTime())
        ? Math.floor((end.getTime() - today.getTime()) / dayMs)
        : null
...
const lineChartOptions = computed(() => {
  const isMissionLine = isAdminLike.value && (stats.value.missionSeries || []).length > 0
