...
const getAllWorkHours = async () => {
  const missionExpr = await resolveWorkHourMissionIdExpr();
  const result = await pool.query(
    `
    SELECT 
  ...
  wh.end_time,
    ...
    LEFT JOIN cases c ON c.id = ${missionExpr}
